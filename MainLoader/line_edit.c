...
BOOLEAN line_edit(CHAR16 *line_in, CHAR16 **line_out, UINTN x_max) {
   ...
   refit_call3_wrapper(gST->ConOut->SetCursorPosition, gST->ConOut, (ConWidth - 71) / 2, ConHeight - 1);
   refit_call2_wrapper(gST->ConOut->OutputString, gST->ConOut,
   ...
   while (!exit) {
      ...
      refit_call3_wrapper(gST->ConOut->SetCursorPosition, gST->ConOut, 0, y_pos);
      refit_call2_wrapper(gST->ConOut->OutputString, gST->ConOut, print);
      refit_call3_wrapper(gST->ConOut->SetCursorPosition, gST->ConOut, cursor, y_pos);

      refit_call3_wrapper(gBS->WaitForEvent, 1, &gST->ConIn->WaitForKey, &index);
      err = refit_call2_wrapper(gST->ConIn->ReadKeyStroke, gST->ConIn, &key);
      if (EFI_ERROR(err))
         continue;
