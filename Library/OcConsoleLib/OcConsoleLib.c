...
OcSetConsoleResolutionForProtocol (
    ...
    if (ModeNumber < 0) {
        #if REFIT_DEBUG > 0
        MsgLog ("WARN: No compatible mode for %ux%u@%u (max: %u) resolution\n", Width, Height, Bpp, SetMax);
        #endif
...
OcSetConsoleModeForProtocol (
    ...
    #if REFIT_DEBUG > 0
    MsgLog (
        "Requesting %ux%u (max: %d) console mode, curr %u, max %u\n",
        Width,
        Height,
        SetMax,
    ...
    if (ModeNumber < 0) {
        #if REFIT_DEBUG > 0
        MsgLog ("WARN: No compatible mode for %ux%u (max: %u) console mode\n", Width, Height, SetMax);
        #endif
    ...
    if (ModeNumber == TextOut->Mode->Mode) {
        ...
        #if REFIT_DEBUG > 0
        MsgLog (
            "Current console mode matches desired mode %u, forcing update\n",
            (UINT32) ModeNumber
        );
        #endif
    }
    ...
    #if REFIT_DEBUG > 0
    MsgLog (
        "Setting mode %u (prev %u) with %ux%u console mode\n",
        (UINT32) ModeNumber,
        (UINT32) TextOut->Mode->Mode,
        Width,
        Height
    ...
    if (EFI_ERROR (Status)) {
        #if REFIT_DEBUG > 0
        MsgLog (
            "Failed to set mode %u with %ux%u console mode\n",
            (UINT32) ModeNumber,
            Width,
            Height
        );
    ...
    #if REFIT_DEBUG > 0
    MsgLog ("Changed console mode to %u\n", (UINT32) TextOut->Mode->Mode);
    #endif
...
OcSetConsoleResolution (
    ...
    if (!EFI_ERROR (Result)) {
        ...
        #if REFIT_DEBUG > 0
        MsgLog ("Found %u handles with GOP\n", (UINT32) HandleCount);
        #endif
        ...
        for (Index = 0; Index < HandleCount; ++Index) {
            ...
            if (EFI_ERROR (Status)) {
                #if REFIT_DEBUG > 0
                MsgLog ("WARN: Missing GOP on console - %r\n", Status);
                #endif
        ...
        #if REFIT_DEBUG > 0
        MsgLog ("Failed to find handles with GOP - %r\n", Result);
        #endif
    }
#else
    ...
    if (EFI_ERROR (Result)) {
        #if REFIT_DEBUG > 0
        MsgLog ("WARN: Missing GOP on ConOut - %r\n", Result);
        #endif
