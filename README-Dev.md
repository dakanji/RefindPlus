...
- **Pointer Priority:** The upstream implementation of pointer priority when both pointer control tokens, `enable_mouse` and `enable_touch`, are active is based on how the tokens appear in the configuration file(s). The last one read in the main configuration file and/or any supplementary/override configuration file will be used and the other diregarded. In RefindPlus, the `enable_mouse` token always takes priority when both tokens are active without regard to the order of appearance in the configuration file(s). This means that to use touch in RefindPlus, the `enable_mouse` token must be disabled in addition to enabling the `enable_touch` token.
