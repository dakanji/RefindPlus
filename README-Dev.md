...
- **Pointer Priority:** The upstream implementation of pointer priority is based on how the pointer control tokens, `enable_mouse` and `enable_touch`, appear in the configuration file when both tokens are active. The last one read in the main configuration file or any supplementary/override configuration file will be used and the other diregarded. In RefindPlus, the `enable_mouse` token always takes priority when both tokens are active regardless of how they appear in the configuration file.
