...
- **Tool Icons:** RefindPlus attempts to consistently apply the principle of icons for items designated as `Tools` only appearing on the main screen when present in the `showtools` configuration token and that when such icons are displayed, this should only be in the second row of items (small icons) and never in the first row (large icons).
- **Dont/Also Scan XYZ:** RefindPlus always maintains program defaults in `Dont/Also Scan` lists. That is, user defined items are always added to such lists and never overwrite program defaults as is the case upstream when `"+"` is not included. This makes specifying this list item redundant in RefindPlus as it is effectively assumed to be always present.
